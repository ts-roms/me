'use client';
import { useEffect, useState } from 'react';
import { ExperienceItem, type Experience } from './ExperienceItem';

export function ExperienceList() {
  const [experiences, setExperiences] = useState<Experience[] | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;
    async function load() {
      try {
        const res = await fetch('/api/experience', { cache: 'no-store' });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = (await res.json()) as Experience[];
        if (!cancelled) setExperiences(data);
      } catch (e: any) {
        if (!cancelled) setError(e?.message ?? 'Failed to load experience');
      }
    }
    load();
    return () => {
      cancelled = true;
    };
  }, []);

  if (error) return <p className="text-sm text-red-600 dark:text-red-400">Failed to load experience: {error}</p>;

  if (!experiences) return <p className="text-sm text-gray-500 dark:text-gray-400">Loading experience…</p>;

  return (
    <div className="grid gap-4">
      {experiences.map((exp, i) => (
        <ExperienceItem key={exp.company + i} exp={exp} index={i} />
      ))}
    </div>
  );
}
